...
MVA_1999_2020 <- read_excel("raw/placebo/MVA_1999_2020.xlsx") %>%
  select(-Notes) %>%
  filter(!is.na(State)) %>%
  filter(Year < 2018)

MVA_1999_2020$Deaths <- as.numeric(MVA_1999_2020$Deaths)
MVA_1999_2020$Deaths[is.na(MVA_1999_2020$Deaths)] <- 5

MVA_1999_2020$Population <- as.numeric(MVA_1999_2020$Population)
MVA_1999_2020$`Crude Rate` <- MVA_1999_2020$Deaths/MVA_1999_2020$Population * 100000

MVA_2018_2023 <- read_excel("raw/placebo/MVA_2018_2023.xlsx") %>%
  select(-Notes)  %>%
  filter(!is.na(State))

MVA_2018_2023$Deaths <- as.numeric(MVA_2018_2023$Deaths)
MVA_2018_2023$Deaths[is.na(MVA_2018_2023$Deaths)] <- 5

MVA_2018_2023$Population <- as.numeric(MVA_2018_2023$Population)
MVA_2018_2023$`Crude Rate` <- MVA_2018_2023$Deaths/MVA_2018_2023$Population * 100000

MVA_1999_2023 <- rbind(MVA_1999_2020, MVA_2018_2023)

MVA_1999_2023 <- MVA_1999_2023 %>%
...
generate_erpo_panel <- function(
    ...
    start_year = 1999,
    end_year   = 2023,
    all_states = unique(MVA_1999_2023$state)
) {
...
MVA_total <- MVA_1999_2023 
# Merge the data
merged_data <- left_join(erpo_panel, MVA_total, by = c("state", "year"))
...
writeLines(MVA_stan_code, "erpo_model.stan")

# 9) Compile the Stan model
MVA_stan_model <- stan_model(file = "erpo_model.stan")

# 10) Find optimization-based initialization values
init_opt <- optimizing(MVA_stan_model, data = MVA_stan_data, 
                       algorithm = "LBFGS", 
                       as_vector = FALSE)
...
print(MVA_stan_fit, pars = c("beta", "delta1", "delta2", "invphi", 
                                 "IRR_0", "IRR_1", "IRR_2", "IRR_3", "IRR_4", "IRR_5"))

# 13) Check convergence with trace plots
mcmc_trace(MVA_stan_fit, pars = c("beta[1]", "beta[2]", "delta1", "delta2", "invphi"))

# 14) Extract posterior samples and create time-varying IRR data
MVA_samples <- extract(MVA_stan_fit)

# Create a data frame for IRR over time
irr_summary <- data.frame(
  year = 0:5,
  median = c(
    median(MVA_samples$IRR_0),
    median(MVA_samples$IRR_1),
    median(MVA_samples$IRR_2),
    median(MVA_samples$IRR_3),
    median(MVA_samples$IRR_4),
    median(MVA_samples$IRR_5)
  ),
  lower = c(
    quantile(MVA_samples$IRR_0, 0.025),
    quantile(MVA_samples$IRR_1, 0.025),
    quantile(MVA_samples$IRR_2, 0.025),
    quantile(MVA_samples$IRR_3, 0.025),
    quantile(MVA_samples$IRR_4, 0.025),
    quantile(MVA_samples$IRR_5, 0.025)
  ),
  upper = c(
    quantile(MVA_samples$IRR_0, 0.975),
    quantile(MVA_samples$IRR_1, 0.975),
    quantile(MVA_samples$IRR_2, 0.975),
    quantile(MVA_samples$IRR_3, 0.975),
    quantile(MVA_samples$IRR_4, 0.975),
    quantile(MVA_samples$IRR_5, 0.975)
  ),
  prob_less_than_1 = c(
    mean(MVA_samples$IRR_0 < 1),
    mean(MVA_samples$IRR_1 < 1),
    mean(MVA_samples$IRR_2 < 1),
    mean(MVA_samples$IRR_3 < 1),
    mean(MVA_samples$IRR_4 < 1),
    mean(MVA_samples$IRR_5 < 1)
  )
)
...
MVA_n_samples <- length(MVA_samples$IRR_0)

# Create a data frame for all posterior samples
irr_samples <- data.frame(
  sample_id = rep(1:MVA_n_samples, 6),
  year = rep(0:5, each = MVA_n_samples),
  irr = c(
    MVA_samples$IRR_0,
    MVA_samples$IRR_1,
    MVA_samples$IRR_2,
